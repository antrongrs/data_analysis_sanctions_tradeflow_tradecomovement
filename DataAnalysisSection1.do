* Hãy cài ghi đè phiên bản mới nhất của jwdid.ado và jwdid_estat.edo tải về từ "https://github.com/friosavila/stpackages/tree/main/jwdid" *

* =================== *
* 0/ Khai báo thư mục *
* =================== *
global path "D:\NCKH-K23-HIEU"
global fig  "$path\FIG - copy"

do "$path\DataGraph.do"

* ============================= *
* 1/ Sacntions_any - Tradeflows *
* ============================= *
use "$path\DID-DATA.dta", clear
...
xtset od year
	...
	* Extended TWFE with JWDID *
		* Treatment Effect *
		jwdid tradeflows, i(od) t(year) g(gvar_sanctions_any)  never method(ppmlhdfe)  tol(1e-2) itol(1e-2) accel(cg)
		estat simple, predict(xb)

		* Event-study *	
		jwdid tradeflows, i(od) t(year) g(gvar_sanctions_any)  never method(ppmlhdfe) hettype(twfe) fevar(ot) tol(1e-2) itol(1e-2) accel(cg)
		jwdid_store_tests
		
		jwdid tradeflows, i(od) t(year) g(gvar_sanctions_any)  never method(ppmlhdfe) hettype(event) tol(1e-2) itol(1e-2) accel(cg)
...
xtset od year
	...
	* Extended TWFE with JWDID *
		* Treatment Effect *
		jwdid tradeflows, i(od) t(year) g(gvar_trade_sanctions) never method(ppmlhdfe)  tol(1e-2) itol(1e-2) accel(cg)
		estat simple, predict(xb)

		* Event-study *	
		jwdid tradeflows, i(od) t(year) g(gvar_trade_sanctions)  never method(ppmlhdfe) hettype(event)  tol(1e-2) itol(1e-2) accel(cg)
...
xtset od year
	...
	* Extended TWFE with JWDID *
		* Treatment Effect *
		jwdid tradeflows, i(od) t(year) g(gvar_nontrade_sanctions_any) never method(ppmlhdfe)  tol(1e-2) itol(1e-2) accel(cg)
		estat simple, predict(xb)

		* Event-study *	
		jwdid tradeflows, i(od) t(year) g(gvar_nontrade_sanctions_any)  never method(ppmlhdfe) hettype(event)  tol(1e-2) itol(1e-2) accel(cg)
