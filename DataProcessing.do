...
* 1/ Xử lý GBDS
use "$path\GSDB_V4_dyadic.dta", clear 
*Sử dụng file GSDB_V4_dyadic để xử lý*
...
save "$path\Sanctions.dta", replace

use "$path\Sanctions.dta", clear
...
save "$path\Sanctions.dta", replace 
*Lưu file Sanctions cho data xử lý từ file GSDB_V4_dyadic*

* 2/ Xử lý CEPII
use "$path\Gravity_V202202.dta", clear
*Sử dụng file Gravity_V202202 để xử lý*
...
save "$path\Gravity.dta", replace
*Lưu file Gravity cho data xử lý từ file Gravity_V202202*

* 3/ Merge
use "$path\Gravity.dta", clear
*Sử dụng file Gravity*

merge 1:1 iso3_o iso3_d year using "$path\Sanctions.dta"
...
* 4/ Tính Business Cycle Comovement
...
save "$path\DATA.dta", replace
save "$path\TradeComov-DATA.dta", replace
*Lưu lại file DATA và TradeComov-DATA*

use "$path\TradeComov-DATA.dta", clear
...
save "$path\DID-DATA.dta", replace
* Lưu lại file DID-DATA *
